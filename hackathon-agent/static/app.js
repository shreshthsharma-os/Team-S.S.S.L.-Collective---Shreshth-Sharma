  ...
  try {
    ...
    if (!res.ok) {
      if (res.status === 401) {
        appendMessage('bot',
          '## No API Key Set\n\n' +
          'Hackify needs a key to power the AI. The easiest option is your **GitHub token** — no OpenAI account required:\n\n' +
          '```powershell\n$env:GITHUB_TOKEN = "ghp_..."\npython app.py\n```\n\n' +
          'Or use an OpenAI key directly:\n\n' +
          '```powershell\n$env:OPENAI_API_KEY = "sk-..."\npython app.py\n```\n\n' +
          'Restart the server after setting either key, then refresh this page.'
        );
      } else {
